...
SharedWavefunction fnocc_einsums(SharedWavefunction ref_wfn, Options &options) {
    std::shared_ptr<Wavefunction> wfn;
    // core print FNOCC-EINSUMS header
    outfile->Printf("\n\n");
    outfile->Printf("        *******************************************************\n");
    outfile->Printf("        *                                                     *\n");
    outfile->Printf("        *                   FNOCC-EINSUMS                     *\n");
    outfile->Printf("        *                                                     *\n");
    outfile->Printf("        *******************************************************\n");
    outfile->Printf("\n\n");
    ...
    if (!options.get_bool("DFCC")) {
        ...
        if (options.get_bool("NAT_ORBS")) {
            auto fno = std::make_shared<fnocc::FrozenNO>(ref_wfn, options);
        ...
        if (!options.get_bool("RUN_CEPA")) {
            auto ccsd = std::make_shared<fnocc::CoupledCluster>(wfn, options);
            timer_on("FNOCC: CC energy");
            ccsd->compute_energy();
            timer_off("FNOCC: CC energy");
            return ccsd;
        } else {
            auto cepa = std::make_shared<fnocc::CoupledPair>(wfn, options);
        ...
        auto fno = std::make_shared<fnocc::DFFrozenNO>(ref_wfn, options);
...
#ifdef GPUCC
        auto ccsd = std::make_shared<fnocc::GPUDFCoupledCluster>(wfn, options);
        timer_on("FNOCC: CC energy");
        ccsd->compute_energy();
        timer_off("FNOCC: CC energy");
#else
        auto ccsd = std::make_shared<fnocc::DFCoupledCluster>(wfn, options);
        timer_on("FNOCC: CC energy");
        ccsd->compute_energy();
        timer_off("FNOCC: CC energy");
#endif
