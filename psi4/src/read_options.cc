...
int read_options(const std::string &name, Options &options, bool suppress_printing) {
    ...
    **Cfour Interface:** Keyword translates into |cfour__cfour_spherical|. -*/
    ...
    options.add_str("SCF_TYPE", "PK", "DIRECT DF MEM_DF DISK_DF PK OUT_OF_CORE CD GTFOCK GTFOCK_DF DFDIRJ DFDIRJ+COSX DFDIRJ+LINK DFDIRJ+SNLINK");
	...
	That is, setting this forced DiskDFJK when SCF_TYPE=DF. Starting with v1.10, changing this affects 
 	|globals__scf_type| = ``CD`` or ``DISK_DF`` but does not force ``DISK_DF`` when given ``DF``. !expert -*/
        ...
        options.add_str("SAD_SCF_TYPE", "DF", "DIRECT DF MEM_DF DISK_DF PK OUT_OF_CORE CD GTFOCK GTFOCK_DF");
    ...
    if (name == "CPHF" || options.read_globals()) {
        ...
        /*- SCF Type
         -*/
        options.add_str("SCF_TYPE", "DIRECT", "DIRECT DF PK OUT_OF_CORE PS INDEPENDENT GTFOCK GTFOCK_DF DFDIRJ+SNLINK DFDIRJ+LINK DFDIRJ+COSX");
